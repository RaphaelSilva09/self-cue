...
const { LOCK_EXEMPT_SELECTOR } = require('./src/click-lock');
const platform = process.platform;

contextBridge.exposeInMainWorld('cue', {
  platform,
  lockExemptSelector: LOCK_EXEMPT_SELECTOR,
  ...
  on: (channel, cb) => {
    const allowed = ['capture:state', 'llm:start', 'llm:token', 'llm:done', 'llm:error', 'status', 'transcript', 'stt:interim', 'stt:final', 'stt:status', 'vad:state', 'applink:consent-request', 'hide:toggle', 'whisper:download-progress', 'whisper:models-changed', 'lock:state', 'session:cleared'];
